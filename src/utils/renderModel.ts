...
class renderModel {
	...
	init() {
		return new Promise(async (reslove) => {
			...
			const modelFile = { filePath: 'threeFile/glb/glb-9.glb', fileType: 'glb', decomposeName: 'transformers_3' }
	...
	setModel(modelFile: SetModelType) {
		const { filePath, fileType, decomposeName } = modelFile
	 ...
	 * @function onSetStorageViewImage 设置全景图(外部)
	 */
	// 设置场景颜色
	...
	// 设置全景图
	onSetSceneViewImage(config: any) {
	...
	// 设置全景图参数
	onSetSceneViewConfig(config: { blurriness: any; intensity: any }) {
		const { blurriness, intensity } = config
		this.scene.backgroundIntensity = intensity
		this.scene.backgroundBlurriness = blurriness
	}
	// 设置全景图(外部)
	onSetStorageViewImage(url: string, type: string) {
	...
	onSetModelMaterial(config: any) {
		...
		// 修改当前材质列表的参数
		const listMesh = this.modelMaterialList.find((v: { uuid: any }) => v.uuid == uuid) || {};
	...
	// 修改材质显隐
	onSetMeshVisibe(config: { uuid: any; visible: any }) {
		const mesh = this.scene.getObjectByProperty('uuid', config.uuid)
		mesh.visible = config.visible
	}

	// 设置模型贴图（模型自带）
	onSetModelMap(map: any) {
		const { mapId, meshName } = map
		const uuid = store.selectMesh.uuid
		const mesh = this.scene.getObjectByProperty('uuid', uuid)
	...
	// 设置模型贴图 (外部) 
	onSetStorageModelMap(url: any, type: string) {
		return new Promise<void>(async (reslove) => {
			const uuid = store.selectMesh.uuid
			const mesh = this.scene.getObjectByProperty('uuid', uuid)
			// 根据 图片类型选择不同的加载器
	...
	onMouseClickModel(event: { clientX: number; clientY: number }) {
		...
		const intersects = intersectsChildren.filter((item: any) => item.object.isMesh && item.object.material)
	...
	initModelMaterial() {
		...
		this.modelMaterialList.forEach((v: any) => {
			v.visible = true
			const originalMaterial = this.originalMaterials.get(v.uuid);
			v.mapId = originalMaterial.userData.mapId
	 ...
	 */
	// 设置辉光效果
	onSetUnrealBloomPass(config: { glow: any; threshold: any; strength: any; radius: any; toneMappingExposure: any; color: any }) {
		...
		if (glow) {
			...
			const shaderColor = passesArray.find((v: { name: string }) => v.name == 'ShaderColor') || {}
			if (!shaderColor.name) {
	...
	// 模型材质可拖拽
	setModelMeshDrag(model: any) {
		const { manageFlage } = model
		if (manageFlage) {
			if (!this.transformControls) {
				this.transformControls = new TransformControls(this.camera, this.renderer.domElement);
				this.transformControls.setMode('translate')
				this.transformControls.addEventListener('dragging-changed', (event: { value: any }) => {
	...
	// 设置控制器类型
	setTransformControlsType(type: string) {
	...
	onDeleteGeometryMesh(uuid: any) {
		// 找到需要删除的材质
		const mesh = this.scene.getObjectByProperty('uuid', uuid)
		this.modelMaterialList = this.modelMaterialList.filter((v: any) => v.uuid != uuid)
		this.glowMaterialList = this.modelMaterialList.map((v: any) => v.name)
		mesh.clear()
